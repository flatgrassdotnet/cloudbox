...
func FetchPackageList(category string, author string, search string, offset int, count int) ([]common.Package, error) {
	var args []any
	q := "SELECT p.id, p.rev, p.type, p.name, p.dataname, p.author, IFNULL(profiles.personaname, \"\"), IFNULL(profiles.avatarmedium, \"\"), p.description, p.time FROM packages p LEFT JOIN profiles ON p.author = profiles.steamid WHERE p.rev = (SELECT MAX(p2.rev) FROM packages p2 WHERE p2.id = p.id)"
